...
AddrSpace::AddrSpace(const AddrSpace* other, PCB* newpcb) {
    ...
    if (numPages <= memoryManager->getNumFreePages()) {
        ...
        // Set up the page table for the child process.  The child's
        // virtual pages are the same numbers as the parent's virtual pages,
        // but every virtual page maps to a newly allocated physical page.
        for (i = 0; i < numPages; i++) {
        ...
        machineLock->Acquire();
        // Copy the bytes from each of the parent's physical pages into the
        // newly allocated physical page for the child.
        for (i = 0; i < numPages; i++) {
            int parentPhysAddr = (other->pageTable)[i].physicalPage * PageSize;
            int childPhysAddr = pageTable[i].physicalPage * PageSize;
            bzero(&(machine->mainMemory[childPhysAddr]), PageSize);
            bcopy(&(machine->mainMemory[parentPhysAddr]),
                  &(machine->mainMemory[childPhysAddr]),
                  PageSize);
        }
...
bool AddrSpace::isValid() {
    return (pcb != NULL && pageTable != NULL);
}
