...
int forkImpl() {
    ...
    PCB* childPCB = new PCB(newPID, currPID);
    childPCB->status = P_RUNNING;
    childPCB->process = childThread;

    // Make the child address space an exact copy of the parent's address
    // space.  The virtual addresses are the same, but AddrSpace allocates
    // new physical pages and copies the bytes page by page.
    childThread->space = new AddrSpace(currentThread->space, childPCB);

    int childNumPages = childThread->space->getNumPages();
    if (!childThread->space->isValid()) {
        fprintf(stderr, "Process %d Fork: start at address 0x%x with %d pages"
               " memory failed\n", currPID, newProcessPC, childNumPages);
        processManager->clearPID(newPID);
        delete childThread->space;
        childThread->space = NULL;
        delete childThread;
        return -1;
    }

    processManager->addProcess(childPCB, newPID);

    // Save the user register state into both the parent thread and the child
    // thread before the scheduler can run another user process.
    ...
    // Set up the function that the child process will run, then let it run.
    childThread->Fork(copyStateBack, newProcessPC);
    currentThread->Yield();

    // When the parent resumes, restore its own user state before returning
...
SpaceId execImpl(char* filename) {
    ...
    int newPID = processManager->getPID();
    if (newPID == -1) {
        fprintf(stderr, "Exec Program: %d could not allocate a PID for %s\n",
                currPID, filename);
    ...
    // Give the new process a fresh address space loaded from the executable.
    ...
    // Close file and execute new process.  Save/restore the parent's user
    // state because the new process may run and change the simulated machine
    // registers before Exec returns to the caller.
    delete fileToExecute;
